...
class Environment(object):
    ...
    def reset(self, mode):
        ...
        if mode == "testing":
            ...
            if not self.remaining_training_episodes:
                self.remaining_training_episodes = deepcopy(self.training_episodes)
            #episode = self.remaining_training_episodes.pop()
    ...
    def get_obs(self):
        ...
        grid_1 = np.zeros((5, 5), np.float32)
        ...
        grid_2[self.target_loc[0], self.target_loc[1]] = 1

        grid_3 = np.zeros((5, 5), np.float32)
        # Set the item locations in the grid
        for item_loc in self.item_locs:
            grid_3[item_loc[0], item_loc[1]] = 1

        grid_4 = np.zeros((5, 5), np.float32)
        # Set the remaining time for each item in the grid
        for item_loc, item_time in zip(self.item_locs, self.item_times):
            grid_4[item_loc[0], item_loc[1]] = (self.max_response_time - item_time) / self.max_response_time

        # Normalize the agent's load (capacity) and stack it as a separate channel
        agent_capacity_channel = np.full((5, 5), (self.agent_capacity - self.agent_load) / self.agent_capacity)
        ...
        if True:
            ...
            #state = np.stack((grid_1, grid_4, agent_capacity_channel), axis=-1)
            #state = np.stack((grid_1, grid_2, grid_3, agent_capacity_channel), axis=-1)
            ...
            state = np.zeros(shape=(27,), dtype=np.float32)
            state[0] = float(self.agent_loc[0] + 5 * self.agent_loc[1] + 1) / 25.0
            state[1] = float(self.agent_capacity - self.agent_load) / float(self.agent_capacity)
            for i, item in enumerate(self.item_locs):
                item_cell = item[0] + 5 * item[1] + 2
                state[item_cell] = 1 # float(self.max_response_time - self.item_times[i]) / self.max_response_time
