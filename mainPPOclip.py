...
# from environment_small_state import Environment # for small_state
from environment_dist_state import Environment
        ...
        for episode in used_data:
            ...
            # Calculate weight updates for policy and value network
                ...
                if ep > 1:
                    ...
                    benchmark = [215, 370, 245]
                    if tot_rew_avg > benchmark[self.env.variant]:
                        print('Converged after ', ep, ' episodes with mean total reward of ', tot_rew_avg)
                        self.agent.actor.save_weights("ppo_actor_weights_215")
                        self.agent.critic.save_weights("ppo_critic_weights_215")
                        return rew[:, :ep+1], ep
                        break
...
class PPO_Agent:
    def __init__(self):
        self.no_of_features = 14 if env.variant == 0 else 19
        ...
        self.max_time_steps = 1000  # Maximum time steps
        ...
        policy_learning_rate = 0.001 # 0.0005 achieved 204 at 397
        value_function_learning_rate = 0.001 # 0.0005 achieved 204 at 397
    ...
    def prepare_training_data(self):
        ...
        tmp = np.reshape(self.state_buffer, (self.max_time_steps*self.actors, self.no_of_features))
    ...
    def nn_model(self, state_size, output_size):
        ...
        dense1 = tf.keras.layers.Dense(units=64, activation=tf.tanh)(observation_input) # tanh achieved 204 at 397
        dense2 = tf.keras.layers.Dense(units=64, activation=tf.tanh)(dense1) # tanh achieved 204 at 397
